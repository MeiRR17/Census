"""
CMS Service - שירות מאוחדת ל-CMS (Cisco Meeting Server)
"""
import logging
...
class CMSService:
    ...
    def __init__(self, base_url: str, username: str, password: str):
        ...
        self.session.verify = False  # לפיתוחון
        
    # HTTP Methods
    def _make_request(self, method: str, endpoint: str, data: Optional[Dict] = None, xml: Optional[str] = None) -> requests.Response:
        """שליחת בקשת HTTP ל-CMS"""
        url = f"{self.base_url}/{endpoint.lstrip('/')}"
        headers = {}
        
        if xml:
            headers['Content-Type'] = 'application/xml'
            response = self.session.request(method, url, data=xml.encode('utf-8'), headers=headers)
        else:
    ...
    # CoSpace Management
    def create_cospace(self, name: str, uri: Optional[str] = None, passcode: Optional[str] = None) -> Dict[str, Any]:
        """יצירת CoSpace חדש"""
        ...
        response = self._make_request('POST', 'coSpaces', xml=xml_data)
    ...
    def delete_cospace(self, cospace_id: str) -> bool:
        """מחיקת CoSpace"""
        response = self._make_request('DELETE', f'coSpaces/{cospace_id}')
        return response.status_code == 204
    
    def update_cospace_passcode(self, cospace_id: str, passcode: str) -> Dict[str, Any]:
        """עדכון קוד גישה ל-CoSpace"""
        xml_data = f'''<?xml version="1.0" encoding="UTF-8"?>
        <coSpace>
            <passcode>{passcode}</passcode>
        </coSpace>'''
        
        response = self._make_request('PUT', f'coSpaces/{cospace_id}', xml=xml_data)
        if response.status_code == 200:
            return self._parse_xml_response(response.text)
        else:
            raise Exception(f"Failed to update CoSpace passcode: {response.status_code} - {response.text}")
    
    def get_cospace_details(self, cospace_id: str) -> Dict[str, Any]:
        """קבלת פרטי CoSpace"""
        response = self._make_request('GET', f'coSpaces/{cospace_id}')
        if response.status_code == 200:
            return self._parse_xml_response(response.text)
        else:
    ...
    def list_cospaces(self) -> List[Dict[str, Any]]:
        """רשימת כל ה-CoSpaces"""
        response = self._make_request('GET', 'coSpaces')
        if response.status_code == 200:
    ...
    def get_call_participants(self, call_id: str) -> List[Dict[str, Any]]:
        """קבלת כל המשתתפים בשיחה"""
        response = self._make_request('GET', f'calls/{call_id}')
        if response.status_code == 200:
            root = ET.fromstring(response.text)
            participants = []
    ...
    def mute_participant(self, call_id: str, participant_name: str, mute: bool = True) -> bool:
        ...
        response = self._make_request('PUT', f'calls/{call_id}/participants/{leg_id}', xml=xml_data)
    ...
    def kick_participant(self, call_id: str, participant_name: str) -> bool:
        ...
        response = self._make_request('DELETE', f'calls/{call_id}/participants/{leg_id}')
        return response.status_code == 204
    
    # System Management
    def get_system_info(self) -> Dict[str, Any]:
        """קבלת מידע על המערכת CMS"""
    ...
    @classmethod
    def create_from_env(cls) -> 'CMSService':
        """יצירת שירות CMS ממשתני סביבה"""
        import os
        
        base_url = os.getenv('CMS_HOST', '')
        username = os.getenv('CMS_USERNAME', '')
        password = os.getenv('CMS_PASSWORD', '')
        
        if not all([base_url, username, password]):
            raise ValueError("Missing CMS configuration in environment variables")
